...
export const History = () => {
  ...
  const {
    ...
    queryKey: ["getAllBillsById", selectedRenter],
    enabled: Boolean(selectedRenter),
    // refetchOnWindowFocus: false,
    queryFn: async () => {
      const res = await axios.get(
        `http://localhost:3000/bills/getBill/${selectedRenter}`,
      );
      return res.data;
    },
  });
  ...
  const columns = [
    ...
    columnHelper.accessor("previous_reading", {
      header: "Previous Unit",
      cell: (info) => info.getValue(),
    }),
    columnHelper.accessor("units_used", {
    ...
    columnHelper.accessor("electricity_bill", {
      header: "Amount",
      cell: (info) => info.getValue(),
    }),
  ...
  return (
    <div className="p-2 w-screen">
      <h1 className="text-3xl">Bills History</h1>
      {/* <div
      ...
      <div className=" overflow-auto">
        <Table data={renterBills?.data ?? []} columns={columns} />
      </div>
    </div>
  );
};
